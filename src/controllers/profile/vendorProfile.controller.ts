...
class VendorProfileController {
  async createVendorProfile(req: Request, res: Response) {
    const {
      ...
      user_id,
      service_locations,
    }: CreateVendorProfileDto = req.body;
    const vendorProfile = await vendorProfileService.createVendorProfile({
      business_name,
      description,
      address,
      phone_number,
  ...
  async createServiceDetails(req: Request, res: Response) {
    ...
    const serviceDetails = await vendorProfileService.createServiceDetails({
      user_id,
      vendor_type_id,
      vendor_categoryids,
    });
  ...
  async createPricingDetails(req: Request, res: Response) {
    const { user_id, pricing_details }: CreatePricingDetailsDto = req.body;
    const pricingDetails = await vendorProfileService.createPricingDetails({
      user_id,
      pricing_details,
    });
  ...
  async uploadWorkGalleryImages(req: Request, res: Response) {
    ...
    const workGalleryImages = await vendorProfileService.uploadWorkGalleryImages({
      user_id,
      image_urls,
    });
