...
class VendorProfileRepository {
    create = async (data:Omit<CreateVendorProfileDto,"service_locations">, tx?: Transaction): Promise<VendorProfile> => {
    ...
    updateProfileStep = async (user_id: number, profile_step: number, profile_completed: boolean, tx?: Transaction): Promise<void> => {
        await VendorProfile.update({ profile_step, profile_completed }, { where: { user_id }, transaction: tx });
    }
}
