...
import { CreatePricingDetailsDto, CreateServiceDetailsDto, CreateVendorProfileDto, PricingDetailsResponseDto, ServiceDetailsResponseDto, UploadWorkGalleryDto, UploadWorkGalleryResponseDto, VendorProfileResponseDto } from "@dto/profile.dto";
...
class VendorProfileService {
    createVendorProfile = async (data: CreateVendorProfileDto): Promise<VendorProfileResponseDto> => {
        ...
        return sequelize.transaction(async (tx) => {
            const vendorProfile = await vendorProfileRepository.create({ user_id, business_name, description, address, phone_number, email }, tx);
    ...
    createServiceDetails = async (data: CreateServiceDetailsDto): Promise<Omit<ServiceDetailsResponseDto, "id">> => {
        ...
        const vendorCategoriesBelongsToVendorType = vendorCategories.every((category) => category.vendor_type_id === vendor_type_id);
        ...
        const alreadyAddedCategoryIds = vendor_categoryids.filter((categoryId) => existingCategoryIdsForUser.includes(categoryId));
        ...
        return sequelize.transaction(async (tx) => {
            ...
            const serviceDetails = await vendorProfileCategoryRepository.create({ vendor_profile_id: vendorProfile.id, vendor_categoryids }, tx);
    ...
    createPricingDetails = async (data: CreatePricingDetailsDto): Promise<PricingDetailsResponseDto> => {
       ...
       const vendorProfile = await vendorProfileRepository.findByUserId(user_id);
        ...
        if(vendorProfile.profile_step !== 3) {
            throw ApiError.badRequest("Complete service details first");
        }
        return sequelize.transaction(async (tx) => {
            const pricingDetails = await vendorPricingRepository.create({ vendor_profile_id: vendorProfile.id, pricing_details, amount }, tx);
    ...
    uploadWorkGalleryImages = async (data: UploadWorkGalleryDto): Promise<UploadWorkGalleryResponseDto> => {
        ...
        return sequelize.transaction(async (tx) => {
            const workGalleryImages = await VendorGalleryRespository.bulkCreate(image_urls.map((image_url) => ({ vendor_profile_id: vendorProfile.id, image_url })), tx);
            await vendorProfileRepository.updateProfileStep(user_id, 4, false, tx);
            return {
                vendor_profile_id: vendorProfile.id,
                image_urls: workGalleryImages.map((w) => ({ id: w.id, image_url: w.image_url })),
            };
        });
    }
}
